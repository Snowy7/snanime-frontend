...
export default function BrowsePageClient() {
    ...
    try {
      ...
      setSearchResults(
        results.results.map((anime) => ({
          ...anime,
          image: anime.banner, // Or a more appropriate field
          nsfw: false, // Assuming a default
        }))
      );
  ...
  const applyFilters = (results: SnAnimeSearchResult[]): SnAnimeSearchResult[] => {
    return results
      .filter((anime) => {
        // Type filter (only filter supported by backend data)
        if (filters.types.length > 0 && !filters.types.includes(anime.type)) {
          return false;
        }
        // You can add more filters here if the backend provides more fields in the future
        return true;
      })
      .sort((a, b) => {
        let comparison = 0;
