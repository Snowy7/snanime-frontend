  ...
  return (
    <Link href={`/anime/${episode.id}`} className="group block overflow-hidden rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300 ">
      <div className="relative w-full aspect-[2/3] overflow-hidden">
        ...
        <Image src={episode.posterUrl} alt={episode.title} fill style={{ objectFit: "cover" }} className="transition-transform duration-300 group-hover:scale-115 group-hover:-rotate-6" />
        ...
        <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end justify-end p-3">
          <div className="flex flex-wrap gap-2">
            ...
            <span className="bg-green-500/20 border border-green-700 backdrop-blur-sm text-white text-xs font-semibold px-2 py-1 rounded-full">{t("episode")} {episode.episodeNumber}</span>
            {episode.rating != "NONE" && episode.rating && <span className={`text-xs font-semibold px-2 py-1 rounded-lg ${getColorFromRating(episode.rating)}`}>{episode.rating}</span>}
            {episode.year && <span className="text-xs font-semibold px-2 py-1 rounded-lg bg-blue-500/20 border border-blue-500 backdrop-blur-sm text-white">{episode.year}</span>}
      ...
      <div className="p-3 md:p-4">
        ...
        <p className="text-xs md:text-sm text-neutral-400 mt-1">{t("episode")} {episode.episodeNumber}</p>
      </div>
    </Link>
  );
};
